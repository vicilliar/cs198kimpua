...
func set_curr_key(i):
	curr_key = i
	var normal_texture = load("res://assets/images/scenes/zen_mode/1_main/keys/" + notes[curr_key] + ".png")
...
func _on_key_touch_button_pressed():
	...
	emit_signal("key_touched", notes[curr_key])
	print(notes[curr_key] + " Touched")
	

func _on_key_touch_button_released():
	# Send to keyboard first!
	emit_signal("key_released", notes[curr_key])
	print(notes[curr_key] + " Released")


func play_key():
	var key_sound = get_node(notes[curr_key] + "_sound")
	key_sound.play()
	print(notes[curr_key] + " Playing")
...
func reskin_key(interval):
	...
	var pressed_texture = load(skins_folder + "pressed_" + notes[curr_key] + ".png")
